...
humans, mono for machines. The code is the spec, in the `iii-hq/workers` monorepo: tokens in `ade/web/src/index.css`,
public CSS recipes in `ade/web/src/styles/ui-recipes.css`, components in
`ade/web/src/components/ui` exported through `@iii-dev/console-ui`. Outside the monorepo the
package's copies are what you have: `node_modules/@iii-dev/console-ui/` (`index.d.ts`,
`component-names.mjs`, `ui-classes.mjs`, `token-names.mjs`, `hooks.d.mts`, `format.d.mts`).
This file is the only canonical text. Workspace behaviour (panes, tabs, chords, palette,
annotations) belongs to the console itself, not here.
